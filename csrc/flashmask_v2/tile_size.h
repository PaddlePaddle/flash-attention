...
constexpr std::tuple<int, int, bool, bool> tile_size_fwd_sm90(
        ...
        SeqlenDispatchTag seqlen_tag=SeqlenDispatchTag::LongSeq) {
    if (element_size == 2) {
        if (seqlen_tag == SeqlenDispatchTag::TinySeq) {
            return {64, 64, false, true};
        }
        if (headdim <= 64) {
            if (seqlen_tag == SeqlenDispatchTag::LongSeq) {
