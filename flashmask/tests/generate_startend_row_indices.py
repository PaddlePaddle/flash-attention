...
def generate_non_causal_mask(batch_size, seqlen_q, seqlen_k, h):
    down_left_row_indices = paddle.to_tensor(np.array([seqlen_k] * seqlen_k), dtype=paddle.int32).reshape((1, 1, seqlen_k, 1)).repeat_interleave(batch_size, 0)
    up_right_row_indices = paddle.to_tensor(np.array([0] * seqlen_k), dtype=paddle.int32).reshape((1, 1, seqlen_k, 1)).repeat_interleave(batch_size, 0)
    startend_row_indices = paddle.concat([down_left_row_indices, up_right_row_indices], axis=-1)
    
    causal = False
    return startend_row_indices, causal

def generate_causal_mask(batch_size, seqlen_q, seqlen_k, h):
    startend_row_indices = np.array([seqlen_k] * seqlen_k)
