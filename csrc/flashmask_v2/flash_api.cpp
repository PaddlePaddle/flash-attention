...
void run_mha_fwd_entry(Flash_fwd_params &params, cudaStream_t stream) {
    flashmaskv2_run_mha_fwd(&params, stream);
}
...
void run_mha_fwd_combine_entry(Flash_fwd_params &params, cudaStream_t stream, bool enable_pdl=false) {
        ...
        flashmaskv2_run_mha_fwd_combine(&params, stream, enable_pdl);
    #else
...
inline bool get_pagedkv_tma_entry(Flash_fwd_params &params) {
    return flashmaskv2_get_pagedkv_tma(&params);
}

inline bool get_pack_gqa_entry(Flash_fwd_params &params) {
    return flashmaskv2_get_pack_gqa(&params);
}

inline int get_num_splits_entry(Flash_fwd_params& params) {
    return flashmaskv2_get_num_splits(&params);
}
...
mha_fwd(const at::Tensor &q,   // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
    ...
    // TORCH_CHECK(is_sm8x, "FLASHMASK_V2 only supports Ampere GPUs or newer.");
    // Paddle: const bool is_sm90 = dprops.major == 9 && dprops.minor == 0;
    bool is_sm90 = dprops->major == 9 && dprops->minor == 0;
    TORCH_CHECK(is_sm90, "FlashAttention-3 (FlashMaskV2) only supports Hopper GPUs (e.g., H100/H800).");
    // 修改 end

    auto q_type = q.scalar_type();
    TORCH_CHECK(q_type == at::ScalarType::Half || q_type == at::ScalarType::BFloat16 || q_type == at::ScalarType::Float8_e4m3fn,
                "FLASHMASK_V2 only supports fp16, bf16, and fp8_e4m3 data type");
    ...
    TORCH_CHECK(head_size <= max_headdim, "FLASHMASK_V2 forward only supports head dimension at most " + std::to_string(max_headdim));
    ...
    set_params_fprop(params,
                    ...
                    //  attention_chunk,
                     ...
                     sm_margin);
    // 疑惑
    // if (startend_row_indices_.has_value()) {
    //     params.flashmask_indices_ptr = startend_row_indices_.value().data_ptr();

    // } else {
    //     params.flashmask_indices_ptr = nullptr;
    // }
    // if (block_mask_.has_value()) {
    //     params.block_mask_ptr = block_mask_.value().data_ptr();
    // } else {
    //     params.block_mask_ptr = nullptr;
    // }
...
#ifdef FLASHMASK_V2_DISABLE_BACKWARD
...
#else
void run_mha_bwd_entry(Flash_bwd_params &params, cudaStream_t stream) {
    flashmaskv2_run_mha_bwd(&params, stream);
}
#endif
