...
def do_bench_flashmaskcp(q_local, k_local, v_local, o_grad_local, startend_row_indices, group, is_causal,mode = "balance_q", warmup=50, rep=100, grad_to_none=None, quantiles=None, fast_flush=True, return_mode="mean"):
    """
    Benchmark the runtime of the provided function. By default, return the median runtime of :code:`fn` along with
    the 20-th and 80-th performance percentile.
        ...
        for idx in range(0,50):
            ...
            for ex in ex_to_run:
                if ex in available_examples:
                    ...
                    results.append([idx, f"{fw_time:.4f}", f"{bw_time:.4f}", f"{total_time:.4f}", f"{balance_time:.4f}", f"{scatter_x_time:.4f}", f"{gather_x_time:.4f}"])
